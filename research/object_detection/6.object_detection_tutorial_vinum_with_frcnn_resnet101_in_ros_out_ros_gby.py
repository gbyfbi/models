...
class ObjectDetector:
    def __init__(self, ):
        ...
        self.left_color_image_topic_name = "/stereo/left_image_raw"
        self.right_color_image_topic_name = "/stereo/right_image_raw"
        ...
        self.right_color_image_buffer = [None] * self.image_buffer_len
        self.image_index = -1
        self.cv_bridge = CvBridge()
        self.display_thread = threading.Thread(target=self.__daemon_display_image)
        self.display_thread.start()
        self.time_synchronizer = TimeSynchronizer((MsgFilterSubscriber(self.left_color_image_topic_name, RosImage),
                                                  MsgFilterSubscriber(self.right_color_image_topic_name, RosImage)), 10)
    ...
    def __callback_on_rgb_and_depth_images(self, left_rgb_ros_image_msg, right_rgb_ros_image_msg):
        ...
        left_rgb_cv_image = self.cv_bridge.imgmsg_to_cv2(left_rgb_ros_image_msg, "rgb8")
        left_bgr_cv_image = cv2.cvtColor(left_rgb_cv_image, cv2.COLOR_RGB2BGR)
        right_rgb_cv_image = self.cv_bridge.imgmsg_to_cv2(right_rgb_ros_image_msg, "rgb8")
        # self.display_image_on_window(rgb_cv_image, "color image")
        self.display_image_on_window(left_bgr_cv_image, "color image")
        self.display_image_on_window(right_rgb_cv_image, "depth image")
        buffer_index = (self.image_index + 1) % self.image_buffer_len
        self.color_image_buffer[buffer_index] = left_rgb_cv_image
        # self.color_image_buffer[buffer_index] = bgr_cv_image
        # self.depth_image_buffer[buffer_index] = right_rgb_cv_image
        self.right_color_image_buffer[buffer_index] = right_rgb_cv_image
        self.image_index += 1
