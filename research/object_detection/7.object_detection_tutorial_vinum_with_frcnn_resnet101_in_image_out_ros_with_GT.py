...
class ObjectDetector:
    def __init__(self, ):
        ...
        self.ground_truth_bnbbox_buffer = [None] * self.image_buffer_len
        self.image_index = -1
        ...
        threading.Timer(1, self.read_rgb_images, ["/exchange/DL_working_dir/object_detection/tf_faster_rcnn/vinum_data/images"]).start()
    ...
    def read_rgb_images(self, rgb_image_dir):
        ...
        def get_bndbox_from_gt_xml(xml_path):
            with open(xml_path, 'r') as f:
                xml_str = f.read()
            xml_obj = etree.fromstring(xml_str)
            width_xml_obj = xml_obj.find('.//width')
            width_float = float(width_xml_obj.text)
            height_xml_obj = xml_obj.find('.//height')
            height_float = float(height_xml_obj.text)

            object_xml_obj_list = xml_obj.findall('object')
            bndbox_xmin_xmax_ymin_ymax_tuple_list = []
            for object_xml_obj in object_xml_obj_list:
                bndbox_xml_obj = object_xml_obj.find('bndbox')
                xmin_xml_obj = bndbox_xml_obj.find('xmin')
                ymin_xml_obj = bndbox_xml_obj.find('ymin')
                xmax_xml_obj = bndbox_xml_obj.find('xmax')
                ymax_xml_obj = bndbox_xml_obj.find('ymax')
                bndbox_xmin_xmax_ymin_ymax_tuple_list.append((
                    float(ymin_xml_obj.text)/height_float,
                    float(xmin_xml_obj.text)/width_float,
                    float(ymax_xml_obj.text)/height_float,
                    float(xmax_xml_obj.text)/width_float,))
            return bndbox_xmin_xmax_ymin_ymax_tuple_list
        ...
        annotations_dir = os.path.join('/exchange/DL_working_dir/object_detection/tf_faster_rcnn/vinum_data', 'annotations')
        ...
        xml_file_dir = "/exchange/DL_working_dir/object_detection/tf_faster_rcnn/vinum_data/annotations/xmls"
        while True:
            # for rgb_image_path in rgb_image_path_list:
            for val_example in val_examples:
                rgb_image_path = os.path.join('/exchange/DL_working_dir/object_detection/tf_faster_rcnn/vinum_data/images', val_example+'.jpg')
                ...
                m = re.match(r".+\/(.+)\.jpg", rgb_image_path)
                image_file_root = m.group(1)
                xml_file_path = os.path.join(xml_file_dir, image_file_root+".xml")
                assert os.path.exists(xml_file_path)
                bndbox_mxin_xmax_ymin_ymax_tuple_list = get_bndbox_from_gt_xml(xml_file_path)
                self.ground_truth_bnbbox_buffer[buffer_index] = np.array(bndbox_mxin_xmax_ymin_ymax_tuple_list)
                self.image_index += 1
                # rospy.sleep(0.08)
                rospy.sleep(3)
            ...
            with tf.Session(graph=detection_graph) as sess:
                while True:
                    if self.image_index >= 0:
                        ...
                        bndbox_np_nx4 = self.ground_truth_bnbbox_buffer[buffer_index]
                        vis_util.visualize_boxes_and_labels_on_image_array(
                            image_np,
                            bndbox_np_nx4,
                            np.squeeze(classes).astype(np.int32),
                            ...
                            line_thickness=2,
                            min_score_thresh=0.5,
                            groundtruth_box_visualization_color='OrangeRed',
                        )
